const SapFlow = {
  ...
  _renderDiagram(flowData, project) {
    ...
    return `
      <div class="sap-flow-container">
        ...
        <div class="sap-flow-tier">
          <div class="sap-flow-tier-label">Internal Orders &amp; WBS Elements <span class="sap-flow-tier-hint">settled to cost centres or directly to CO-PA</span></div>
          <div class="sap-flow-tier-row">
            ${internalOrders.map(o => renderObj(o, '#7c3aed')).join('')}
            ${wbsElements.map(o => renderObj(o, '#c2410c')).join('')}
          </div>
          <div class="sap-flow-flow-arrow" style="color:#7c3aed">↑ KO88 / CJ88 settlement</div>
        </div>` : ''}
  ...
  _renderEventTable(project, flowData) {
    ...
    const relevantEvents = (project.events || []).filter(e =>
      e.ficoScope || e.archetypeId || e.sapModule ||
      (e.columns || []).some(c => c.ownerId)
    );
    ...
    const rows = relevantEvents.map(e => {
      ...
      const ownerIds = [...new Set((e.columns || []).map(c => c.ownerId).filter(Boolean))];
      const owners = ownerIds.map(id => {
        const obj = (project.costObjects || []).find(o => o.id === id);
        return obj ? this._esc(obj.objectId) : id;
      }).join(', ');
      ...
      return `
        <tr onclick="Projects.renderEventDetail('${project.id}','${e.id}',null,null)" style="cursor:pointer">
          ...
          <td>${e.sapModule ? `<code style="font-size:11px">${this._esc(e.sapModule)}</code>` : '—'}</td>
          <td style="font-size:11px">${owners || '—'}</td>
